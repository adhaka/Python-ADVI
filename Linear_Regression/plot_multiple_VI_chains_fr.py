...
real log_density;
#log_density = normal_lpdf(y|X*w, sigma) + normal_lpdf(w| 5, 1) + gamma_lpdf(sigma|0.5, 0.5) + log(sigma);
log_density = normal_lpdf(y|X*w, sigma) + normal_lpdf(w| 5, 1);
}
...
itt_max_list = [5000, 10000, 15000, 20000, 25000]
...
for i, itt_max in enumerate(itt_max_list):
    ...
    for j in range(num_K):
        ...
        regression_data= data_generator_linear(N, K, noise_sigma=noise_sigma, mode=p, seed=SEED)
        ...
        try:
            sm = pickle.load(open('pickled_results/model_linear_reg_chains72.pkl', 'rb'))
        except:
            sm = pystan.StanModel(model_code=linear_reg_fixed_variance_code)
            with open('pickled_results/model_linear_reg_chains72.pkl', 'wb') as f:
                pickle.dump(sm, f)
        ...
        posterior_mean = np.diag(posterior_mean)
...
plt.savefig('plots/vi_chains_FR_15D.pdf')
...
plt.legend()
plt.savefig('plots/vi_chains_sigma_0_FR_15D.pdf')
...
plt.legend()
plt.savefig('plots/R_hat_vs_iters_FR_15D.pdf')
...
plt.legend()
plt.savefig('plots/K_hat_vs_iters_FR_15D.pdf')
plt.figure()
...
plt.yscale('log')
plt.legend()
plt.savefig('Lin_Reg_FR_15D_2000N_hmc_Gelbo.pdf')
...
plt.yscale('log')
plt.legend()
plt.savefig('Lin_Reg_FR_mean_15D_2000N_hmc_Gelbo.pdf')
#plt.show()
...
plt.yscale('log')
plt.legend()
plt.savefig('Lin_Reg_FR_sigmas_15D_2000N_hmc_Gelbo.pdf')
